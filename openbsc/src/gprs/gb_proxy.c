				   ...
				   struct gbproxy_peer *peer)
{
	...
	if (tlli_info && tlli_info->imsi_acq_pending && parse_ctx.g48_hdr &&
		...
		while ((stored_msg = msgb_dequeue(&tlli_info->stored_msgs))) {
			...
			rc = gbprox_relay2sgsn(cfg, stored_msg, peer,
					       msgb_bvci(msg));
...
/* Receive an incoming data message from a BSS-side NS-VC */
static int gbprox_rx_data_from_bss(struct gbproxy_config *cfg,
...
/* Receive an incoming data message from a SGSN-side NS-VC */
static int gbprox_rx_data_from_sgsn(struct gbproxy_config *cfg,
				    struct msgb *msg, uint16_t nsei,
				    uint16_t nsvci, uint16_t ns_bvci)
{
	struct gbproxy_peer *peer;
		...
		uint16_t ns_bvci, uint16_t nsvci)
{
	...
	if (ns_bvci == 0 || ns_bvci == 1) {
		...
		if (remote_end_is_sgsn)
			rc = gbprox_rx_data_from_sgsn(cfg, msg, nsei, nsvci,
						      ns_bvci);
		else
			rc = gbprox_rx_data_from_bss(cfg, msg, nsei, nsvci,
						     ns_bvci);
	}
